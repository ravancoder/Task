      ...
      {fullitem.map(i => ( 
        ...
        <div  className='d-flex justify-content-between my-2  p-2'>
            ...
            <div className='counter d-flex justify-content-end align-items-center mb-1'>
                <Button variant="outline-primary" className=" fs-6" onClick={() => dispatch(addProducts(i.id))} >+</Button>
                 <strong className='p-2'>{i.qty}</strong>
                <Button variant="outline-primary" className=" fs-6"  onClick={() => dispatch(removeProducts(i.id))}>-</Button>
            </div>
            <p className='text-secondary text-end mb-0'>Item price is £{i.price} * {i.qty} = £{(i.ogPrice).toFixed(2)}</p>
             
            </div>
           
        </div>
          {bread && soup || cheese || butter ? <p  className='text-end mb-0  fw-bold text-danger pb-1 border-bottom'>Savings £{(i.itemSavings).toFixed(2)}</p> : ""}
          <p className='text-end mb-0 fs-6 fw-bold  p-1 border-bottom'>Item Cost £{(i.offerPrice).toFixed(2)}</p>    
        </div>

      ))}
      <ul className='list-unstyled' >
        <li className='d-flex justify-content-between fs-4 fw-bold p-2 my-2'><strong>Sub Total:</strong> <span>£{(subtotal).toFixed(2)}</span></li>
        <li className='d-flex justify-content-between fs-4 fw-bold p-2 mb-2'><strong>Savings:</strong><span>£{(totalsavings).toFixed(2)}</span></li>
        <li className='d-flex justify-content-between fs-4 fw-bold p-2 mb-2'><strong>Total Amount:</strong><span>£{(total).toFixed(2)}</span></li>
      </ul>
    </div>
  )
}
